...
**Fast search in large monorepos** *(v1.0.79+)*: In large monorepos, the CLI now uses [tgrep](https://github.com/microsoft/tgrep) — a trigram-indexed grep — instead of ripgrep for file searches. Tgrep builds an index of your codebase and uses it to answer regex queries significantly faster than line-by-line scanning, which makes tools like `@codebase`, `/search`, and context gathering noticeably snappier in very large repositories. No configuration is needed; the CLI switches automatically based on repository size.
